...
import RestList from './RestList/index'
...
class App extends Component {
  ...
  async componentDidMount() {   
    const response = await axios.get("http://localhost:8080/api/restaurant.json", { crossdomain: true });
    const data_ = response.data;
    const RestaurantsCount = data_.Restrants.length;
    const hasNxt = (RestaurantsCount === 0) ? false : true;
    const response1 = await axios.get("http://localhost:8080/api/cuisines.json", { crossdomain: true });
  ...
  render() {
    ...
    const handlePageDec = async () => {
      if (this.state.page > 1) {
        ...
        var elementsCount = document.getElementById("RestResult").childElementCount;
    ...
    const handleSearch = async () => {
      ...
      const r = await axios.post("http://localhost:8080/restaurant/search", {
        obj
      });
    ...
    return (
      <div className="App">
        ...
        <section className={"container"} >
          <Row>
            <Col md={{ span: 10 }} >
              <input className={"g inp"} value={this.state.search_rest} onChange={(e) => handleRest(e)} placeholder={"Restaurant   "} /> </Col>
            <Col md={{ span: 10 }} >
            <AutoComplete className={"g inp"}
              style={{ padding:"0px" }}
            ...
            <Col md={{ span: 4 }} >
              <button className={"write"} onClick={handleSrch}>Search</button>
            </Col>
          </Row>
